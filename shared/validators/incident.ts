...
const IncidentValidator = ({
  ...
  message,
  messages,
}: IncidentValidatorInput): string | false => {
  if (!title) {
...
export interface IncidentMessageValidatorInput {
  message: string;
  status: string;
  monitorIds: string[];
}

export const incidentMessageValidator = ({
  message,
  status,
  monitorIds,
}: IncidentMessageValidatorInput): string | false => {
  if (!message) {
